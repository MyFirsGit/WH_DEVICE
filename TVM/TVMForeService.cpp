...
void CTVMForeService::OnCoinReceived(UINT res,CCHAccept* chAccept)
{
	...
	//	// 读硬币循环HOPPER箱的计数信息，更新，并重新写入。
	//	CTCoinCountInfo::COIN_HOPPER_BOX_INFO& boxInfo = theCOIN_COUNT.GetCoinHopperboxCInfo();
	//	boxInfo.ulCurCount = m_response->coin_info.stock1;
	//	if(!theCOIN_COUNT.SetCoinHopperboxCInfo(boxInfo)) {
	//		theEXCEPTION_LOG.WriteData(CExceptionLog::LEVEL_WARNING,0,_T(__FILE__),__LINE__,_T("循环找零HOPPER箱的计数信息写入失败"));
	//	}
...
bool CTVMForeService::CalculateChangeCount()
{
	...
	//	return false;
	//}
	//CTCoinCountInfo::COIN_HOPPER_BOX_INFO& stockInfo = theCOIN_COUNT.GetCoinHopperboxCInfo();
	//CTCoinCountInfo::COIN_HOPPER_BOX_INFO& hopper1Info = theCOIN_COUNT.GetCoinHopperboxAInfo();
	//CTCoinCountInfo::COIN_HOPPER_BOX_INFO& hopper2Info = theCOIN_COUNT.GetCoinHopperboxBInfo();
