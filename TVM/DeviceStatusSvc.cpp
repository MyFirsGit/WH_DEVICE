...
void CDeviceStatusSvc::ReadCashBoxID()
{
	...
	// ****** 1. 硬币
	CTCoinCountInfo::COIN_HOPPER_BOX_INFO coinHopperBoxAInfo =  theCOIN_COUNT.GetCoinHopperboxAInfo();
	CTCoinCountInfo::COIN_HOPPER_BOX_INFO coinHopperBoxBInfo =  theCOIN_COUNT.GetCoinHopperboxBInfo();
...
void CDeviceStatusSvc::ReadEmptyFull()
{
	...
	try{
		...
		map<BNR_MODULE_ID,tBncCashBoxValueInfo> mapItems = BANKNOTE_HELPER->GetEmptyFull();

		map<BNR_MODULE_ID,tBncCashBoxValueInfo>::iterator it = mapItems.find(BNR_RECYCLER_BOX_A);
		if(it != mapItems.end()){
			m_DataModel.emptyfull.BNR_RECYCLER_BOX_A_Full.Format(_T("%d"),it->second.uiBillMaxNum);
			m_DataModel.emptyfull.BNR_RECYCLER_BOX_A_NearlyFull.Format(_T("%d"),1);
		}

		it = mapItems.find(BNR_RECYCLER_BOX_B);
		if(it != mapItems.end()){
			m_DataModel.emptyfull.BNR_RECYCLER_BOX_B_Full.Format(_T("%d"),it->second.uiBillMaxNum);
			m_DataModel.emptyfull.BNR_RECYCLER_BOX_B_NearlyFull.Format(_T("%d"),1);
		}

		it = mapItems.find(BNR_RECYCLER_BOX_C);
		if(it != mapItems.end()){
			m_DataModel.emptyfull.BNR_RECYCLER_BOX_C_Full.Format(_T("%d"),it->second.uiBillMaxNum);
			m_DataModel.emptyfull.BNR_RECYCLER_BOX_C_NearlyFull.Format(_T("%d"),1);
		}

		it = mapItems.find(BNR_RECYCLER_BOX_D);
		if(it != mapItems.end()){
			m_DataModel.emptyfull.BNR_RECYCLER_BOX_D_Full.Format(_T("%d"),it->second.uiBillMaxNum);
			m_DataModel.emptyfull.BNR_RECYCLER_BOX_D_NearlyFull.Format(_T("%d"),1);
		}
	}
	catch(...){
