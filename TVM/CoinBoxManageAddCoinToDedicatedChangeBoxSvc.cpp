...
void CCoinChangeBoxAddingSvc::InitializeDataModel()
{
	...
	CTCoinCountInfo::COIN_HOPPER_BOX_INFO ctBoxInfo;
	...
	try{
		...
		if(m_DataModel.opStatus == OP_STATUS_RUNING_ADDING){
			...
			if (m_DataModel.mapBoxInfo[CHANGE_BOX_A].AddingCount > 0){
				...
				CTCoinCountInfo::COIN_HOPPER_BOX_INFO boxInfo =  theCOIN_COUNT.GetCoinHopperboxAInfo();
			...
			if (m_DataModel.mapBoxInfo[CHANGE_BOX_B].AddingCount > 0){
				...
				CTCoinCountInfo::COIN_HOPPER_BOX_INFO boxInfo =  theCOIN_COUNT.GetCoinHopperboxBInfo();
...
void CCoinChangeBoxAddingSvc::PrintCoinBoxExchangeReceipt(bool bBoxA)
{
	try
	{
		...
		CTCoinCountInfo::COIN_HOPPER_BOX_INFO operate_hopper_before;
		CTCoinCountInfo::COIN_HOPPER_BOX_INFO operate_hopper_after;
		if (bBoxA){
