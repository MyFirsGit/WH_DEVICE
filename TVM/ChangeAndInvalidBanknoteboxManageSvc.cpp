...
LRESULT CChangeAndInvalidBanknoteboxManageSvc::OnKeyboardF2(WPARAM , LPARAM)
{
	...
	if (cashUnit[BNR_RECYCLER_BOX_A].uiBillNum > m_nRBox1)
	{
		nChangeCount -= (cashUnit[BNR_RECYCLER_BOX_A].uiBillNum - m_nRBox1);
		rabA.ulCurCount = cashUnit[BNR_RECYCLER_BOX_A].uiBillNum;
		theBANKNOTE_COUNT.SetBanknoteRecyclerBoxAInfo(rabA);
	}
	if (cashUnit[BNR_RECYCLER_BOX_B].uiBillNum > m_nRBox2)
	{
		nChangeCount -= (cashUnit[BNR_RECYCLER_BOX_B].uiBillNum - m_nRBox2);
		rabB.ulCurCount = cashUnit[BNR_RECYCLER_BOX_B].uiBillNum;
		theBANKNOTE_COUNT.SetBanknoteRecyclerBoxBInfo(rabB);

	}
	if (cashUnit[BNR_RECYCLER_BOX_C].uiBillNum > m_nRBox3)
	{
		nChangeCount -= (cashUnit[BNR_RECYCLER_BOX_C].uiBillNum - m_nRBox3);
		rabC.ulCurCount = cashUnit[BNR_RECYCLER_BOX_C].uiBillNum;
		theBANKNOTE_COUNT.SetBanknoteRecyclerBoxCInfo(rabC);

	}
	if (cashUnit[BNR_RECYCLER_BOX_D].uiBillNum > m_nRBox4)
	{
		nChangeCount -= (cashUnit[BNR_RECYCLER_BOX_D].uiBillNum - m_nRBox4);
		rabD.ulCurCount = cashUnit[BNR_RECYCLER_BOX_D].uiBillNum;
