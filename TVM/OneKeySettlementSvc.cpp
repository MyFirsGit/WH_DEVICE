...
bool COneKeySettlementSvc::CoinCollection()
{
	...
	CTCoinCountInfo::COIN_HOPPER_BOX_INFO			  mEx_hopper_A = theCOIN_COUNT.GetCoinHopperboxAInfo();	
	CTCoinCountInfo::COIN_HOPPER_BOX_INFO			  mEx_hopper_B = theCOIN_COUNT.GetCoinHopperboxBInfo();
	...
	try
	{
		...
		CTCoinCountInfo::COIN_HOPPER_BOX_INFO hopper_A = theCOIN_COUNT.GetCoinHopperboxAInfo();	
		CTCoinCountInfo::COIN_HOPPER_BOX_INFO hopper_B = theCOIN_COUNT.GetCoinHopperboxBInfo();
...
bool COneKeySettlementSvc::CollectCoin()
{
	...
	// 2. 检查软计数
	CTCoinCountInfo::COIN_HOPPER_BOX_INFO hopperA = theCOIN_COUNT.GetCoinHopperboxAInfo();// 五角
	CTCoinCountInfo::COIN_HOPPER_BOX_INFO hopperB = theCOIN_COUNT.GetCoinHopperboxBInfo();// 一元
	...
	try
	{
		if(BANKNOTE_HELPER->IsCashBoxAttached()){
			if(dwLaftCount > 0 && nBillFullCnt > 0){
				...
				long lErrCode = 0;
				map<BNR_MODULE_ID,tBncCashBoxValueInfo> cashUnit = BANKNOTE_HELPER->GetEmptyFull();
				lErrCode = BANKNOTE_HELPER->Empty((char*)"R",&result);

				if(recyclerA.ulCurCount > 0|| cashUnit[BNR_RECYCLER_BOX_A].uiBillNum > 0){
					if(lErrCode == 0){
				...
				if(nBillFullCnt > 0 && (recyclerB.ulCurCount > 0 || cashUnit[BNR_RECYCLER_BOX_B].uiBillNum > 0)){
				...
				if(nBillFullCnt > 0 && (recyclerC.ulCurCount > 0 || cashUnit[BNR_RECYCLER_BOX_C].uiBillNum > 0)){
				...
				if(nBillFullCnt > 0 && (recyclerD.ulCurCount > 0 || cashUnit[BNR_RECYCLER_BOX_D].uiBillNum > 0)){
				...
				if (nBillFullCnt > 0 && (changeBox.ulCurCount > 0 || cashUnit[BNR_MODULE_CHANGE_BOX].uiBillNum > 0))
				{
