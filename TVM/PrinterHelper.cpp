...
{
	long l_lret = 1;
	l_lret = m_PrinterCmd.PRT_GetStatus();
...
long CPrinterHelper::PrintReceiptByText(CStringArray* sArray, bool bIsSaveFile,bool bIsPrintLogo)
{
	...
	if(theTVM_SETTING.GetPrintMode() == 0){
		lret = m_PrinterCmd.PRT_PrintText(sArray, false);
		if(0 != lret){
...
BOOL CPrinterHelper::isNoPaper()
{
	...
	long l_lret = m_PrinterCmd.PRT_GetStatus();
	if(l_lret==CPrinterException::ERROR_Printer_NO_PAPER)
	{
		bret=TRUE;
	}
...
BOOL CPrinterHelper::isLessPaper()
{
	...
	long l_lret = m_PrinterCmd.PRT_GetStatus();
	if(l_lret==CPrinterException::ERROR_Printer_LESS_PAPER)
	{
		bret=TRUE;
	}
...
BOOL CPrinterHelper::isKnifePosError()
{
	...
	long l_lret = m_PrinterCmd.PRT_GetStatus();
	if(l_lret==CPrinterException::ERROR_Printer_KNIFE_ERROR)
	{
		bret=TRUE;
	}
	return bret;
