...
public:
	typedef enum _tagTimers
	{
		...
		TIMER_WEB_LOCAL_PROCESS	= 12,	// 二维码订单本地处理定时器(未使用)
		TIMER_PARAMETER_UPDATE	= 13,	// SC下载参数加载定时器
	}TIMERS;
	typedef enum _svc_return
	{
		SVC_RET_NORMOL = 0,			// 正常服务
		SVC_RET_OUT_OF_SERVICE = 1,	// 暂停服务
	}SVC_RETURN;
...
public:
	...
	CTVMRWHelper* GetRechargeHelper();
...
private:
	// 通过TvmSetting.ini判断是否支持自动降级
	bool bIsAllowDegrade;
	// 对DefaultService进行模块拆分 王峰@20190409
	// 硬币模块状态分析
	SVC_RETURN CoinModleAnalyse(WORD &device_pay_status,WORD &device_change_status);
	// 纸币模块状态分析
	SVC_RETURN BankNoteModleAnalyse(WORD& device_pay_status, WORD& device_change_status, WORD& device_service_mode);
	// 票卡模块状态分析
	SVC_RETURN TicketModleAnalyse(WORD& device_service_mode);
	// 充值读写器状态分析
	SVC_RETURN RechargeRWModleAnalyse(WORD& device_service_mode);
	// 票卡读写器状态分析
	SVC_RETURN CardRWModleAnalyse(WORD& device_service_mode);
	// 打印机状态分析
	SVC_RETURN PrinterModleAnalyse(WORD& device_print_mode);
	// Web平台状态分析
	SVC_RETURN WebServiceAnalyse(WORD& device_pay_status, WORD& device_service_mode);
};
