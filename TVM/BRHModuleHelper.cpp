...
long CBRHModuleHelper::Init(bool bRepareCount)
{
	...
	if (Status.iErrorCode != BNC_NO_ERROR) 
	{
		Reset();		
	}
	...
	do{
		map<BNR_MODULE_ID,tBncCashBoxValueInfo> cashUnit;
		try{
			cashUnit = BANKNOTE_HELPER->GetEmptyFull();
			map<BNR_MODULE_ID,tBncCashBoxValueInfo>::iterator it = cashUnit.find(BNR_RECYCLER_BOX_A);
			if(it != cashUnit.end()){
				theAPP_SESSION.SetBNRRecyclerNearEmpty(1, it->second.uiBillMaxNum);
			}
			it = cashUnit.find(BNR_RECYCLER_BOX_B);
			if(it != cashUnit.end()){
				theAPP_SESSION.SetBNRRecyclerNearEmpty(2, it->second.uiBillMaxNum);
			}
			it = cashUnit.find(BNR_RECYCLER_BOX_C);
			if(it != cashUnit.end()){
				theAPP_SESSION.SetBNRRecyclerNearEmpty(3, it->second.uiBillMaxNum);
			}
			it = cashUnit.find(BNR_RECYCLER_BOX_D);
			if(it != cashUnit.end()){
				theAPP_SESSION.SetBNRRecyclerNearEmpty(4, it->second.uiBillMaxNum);
			}
			...
			if(bRepareCount){
				// 检查软计数和硬件箱子中实际数量
				if (cashUnit[BNR_RECYCLER_BOX_A].uiBillNum != recycle_A.ulCurCount){
					CString strLog;
					strLog.Format(_T("发现纸币循环鼓A软计数与实际不一致！软计数中%d张，实际%d张;补充箱当前%d张"), recycle_A.ulCurCount, cashUnit[BNR_RECYCLER_BOX_A].uiBillNum, changeBoxInfo.ulCurCount);
					...
					int adjustCount = cashUnit[BNR_RECYCLER_BOX_A].uiBillNum - recycle_A.ulCurCount;
					if(adjustCount>0){//实际比软计数大，调整计数
						changeBoxInfo.ulCurCount -= adjustCount;
					}
					recycle_A.ulCurCount = cashUnit[BNR_RECYCLER_BOX_A].uiBillNum;
				}
				if (cashUnit[BNR_RECYCLER_BOX_B].uiBillNum != recycle_B.ulCurCount){
					CString strLog;
					strLog.Format(_T("发现纸币循环鼓B软计数与实际不一致！软计数中%d张，实际%d张;补充箱当前%d张"), recycle_B.ulCurCount, cashUnit[BNR_RECYCLER_BOX_B].uiBillNum, changeBoxInfo.ulCurCount);
					theEXCEPTION_MGR.WriteExceptionLog(CInnerException(CInnerException::MODULE_ID, CInnerException::OTHER_ERR, _T(__FILE__), __LINE__, strLog));
					int adjustCount = cashUnit[BNR_RECYCLER_BOX_B].uiBillNum - recycle_B.ulCurCount;
					if(adjustCount>0){//实际比软计数大，调整计数
						changeBoxInfo.ulCurCount -= adjustCount;
					}
					recycle_B.ulCurCount = cashUnit[BNR_RECYCLER_BOX_B].uiBillNum;
				}
				if (cashUnit[BNR_RECYCLER_BOX_C].uiBillNum != recycle_C.ulCurCount){
					CString strLog;
					strLog.Format(_T("发现纸币循环鼓C软计数与实际不一致！软计数中%d张，实际%d张;补充箱当前%d张"), recycle_C.ulCurCount, cashUnit[BNR_RECYCLER_BOX_C].uiBillNum, changeBoxInfo.ulCurCount);
					theEXCEPTION_MGR.WriteExceptionLog(CInnerException(CInnerException::MODULE_ID, CInnerException::OTHER_ERR, _T(__FILE__), __LINE__, strLog));
					int adjustCount = cashUnit[BNR_RECYCLER_BOX_C].uiBillNum - recycle_C.ulCurCount;
					if(adjustCount>0){//实际比软计数大，调整计数
						changeBoxInfo.ulCurCount -= adjustCount;
					}
					recycle_C.ulCurCount = cashUnit[BNR_RECYCLER_BOX_C].uiBillNum;
				}
				if (cashUnit[BNR_RECYCLER_BOX_D].uiBillNum != recycle_D.ulCurCount){
					CString strLog;
					strLog.Format(_T("发现纸币循环鼓D软计数与实际不一致！软计数中%d张，实际%d张;补充箱当前%d张"), recycle_D.ulCurCount, cashUnit[BNR_RECYCLER_BOX_D].uiBillNum, changeBoxInfo.ulCurCount);
					theEXCEPTION_MGR.WriteExceptionLog(CInnerException(CInnerException::MODULE_ID, CInnerException::OTHER_ERR, _T(__FILE__), __LINE__, strLog));
					int adjustCount = cashUnit[BNR_RECYCLER_BOX_D].uiBillNum - recycle_D.ulCurCount;
					if(adjustCount>0){//实际比软计数大，调整计数
						changeBoxInfo.ulCurCount -= adjustCount;
					}
					recycle_D.ulCurCount = cashUnit[BNR_RECYCLER_BOX_D].uiBillNum;
				}
...
long CBRHModuleHelper::CheckAndOpenBHComport(){
	...
	logPath = logPath.TrimRight(_T("\\")) + _T("\\");
	CString logName = _T("BNCApi_");
	CBRHOpen brhCmd(port,baud,logPath, logName);
	errCode = brhCmd.ExecuteCommand();
	if(errCode!=BNC_NO_ERROR){
		ProcessErrorCode(errCode);
	}
...
map<BNR_MODULE_ID,tBncCashBoxValueInfo> CBRHModuleHelper::GetEmptyFull(){
	//BRH_RSP_QUERY_CASHUNIT stCashUnit;
	tBncDevReturn    status;
	//map<BNR_MODULE_ID,T_XfsPhysicalCashUnit> mapItems;
	map<BNR_MODULE_ID,tBncCashBoxValueInfo> mapItems;
	...
	// 更新关于回收箱的数量
	for(int i = 0 ;i < MAX_BOXNUM; i++){
		tBncCashBoxValueInfo item = queryCashUnit.pCashBoxValueInfo[i];

		if(item.uiBoxType==1 && item.uiBoxNo==2){
			mapItems[BNR_RECYCLER_BOX_A] = item;
		}
		else if(item.uiBoxType==1 && item.uiBoxNo==3){
			mapItems[BNR_RECYCLER_BOX_B] = item;
		}
        else if(item.uiBoxType==2 && item.uiBoxNo==4){
			mapItems[BNR_RECYCLER_BOX_C] = item;
		}
        else if(item.uiBoxType==2 && item.uiBoxNo==5){
			mapItems[BNR_RECYCLER_BOX_D] = item;
		}
        else if(item.uiBoxType==0 && item.uiBoxNo==0){
			mapItems[BNR_MODULE_CASH_BOX] = item;
		}
		//else if(item.uiBoxType==5 && item.uiBoxNo==4){
