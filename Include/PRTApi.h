...
AFC_API_DLL_IMP int  PRT_Init(tPRTDevReturn*  pDevStatus);
//在设备故障或从维修模式退出时（对于支持维修模式的情况），执行本函数
AFC_API_DLL_IMP int  PRT_Reset(tPRTDevReturn*  pDevStatus);
...
AFC_API_DLL_IMP int  PRT_Print(CStringArray* m_sArrPrinter,bool m_bOpenBox,bool bCutPaper,tPRTDevReturn *pDevStatus);
//功能说明：调用此接口可获取模块当前状态。
//	参数说明见下表： 
...
AFC_API_DLL_IMP int  PRT_GetDevStatus(tPRTModuleStatus *pModuleStatus,tPRTDevReturn *pDevStatus);
...
//	3	tPRTDevReturn *	pDevStatus 	Out	返回状态信息 
AFC_API_DLL_IMP int  PRT_GetVersion(char cFirmWareVersion[32], char cDriverVersion[32], tPRTDevReturn *pDevStatus);

AFC_API_DLL_IMP int  PRT_CommOpen(UINT uiCommPort, UINT uiBaudRate);
AFC_API_DLL_IMP int  PRT_CommClose();
