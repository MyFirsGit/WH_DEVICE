...
// 动作类型：上升；下降
typedef enum _th_action_type_{
	TH_TRAY_UP		= 0x02,  //下降
	TH_TRAY_DOWN	= 0x01,  //上升
}TH_ACTION_TYPE;
...
typedef struct _th_cmd_comm_open
{
	UINT uiCommPort;    // 端口号
	UINT uiBaudRate;    // 波特率
	CString szlogPath;	// 日志目录
	CString szlogName;	// 日志名称
...
//通讯建立
AFC_API_DLL_IMP int TIM_CommOpen(UINT uiCommPort, UINT uiBaudRate, CString szlogPath, CString szlogName);
//模块初始化
AFC_API_DLL_IMP int TIM_Init(char cClearMode,UINT* uiRetractNum,tTIMModuleStatus* pModuleStatus,tTIMDevReturn* pDevStatus);
//模块复位
AFC_API_DLL_IMP int(TIM_Reset)(char cClearMode, UINT* uiRetractNum,tTIMModuleStatus* pModuleStatus,tTIMDevReturn* pDevStatus);
//读取设备驱动版本信息
AFC_API_DLL_IMP int(TIM_GetVersion)(char cFirmWareVersion[32], char cDriverVersion[32], tTIMDevReturn* pDevStatus);
//读取设备状态
AFC_API_DLL_IMP int(TIM_GetDevStatus)(tTIMModuleStatus* pModuleStatus,tTIMDevReturn* pDevStatus);
//获取票卡
AFC_API_DLL_IMP int(TIM_CardOut)(char cBoxNo, tTIMModuleStatus* pModuleStatus,UINT* uiTransportNum, tTIMDevReturn* pDevStatus);
//送出票卡
AFC_API_DLL_IMP int(TIM_SendCard)(tTIMModuleStatus* pModuleStatus,UINT* uiTransportNum, tTIMDevReturn* pDevStatus);
//回收票卡
AFC_API_DLL_IMP int(TIM_RetractCard)(tTIMModuleStatus* pModuleStatus,UINT* uiRetractNum,tTIMDevReturn* pDevStatus);
//读取审计累计值
AFC_API_DLL_IMP int(TIM_GetAuditData)(tTIMAuditCardNum* pAuditCardNum,tTIMDevReturn *pDevStatus);
//读取票箱RFID信息
AFC_API_DLL_IMP int(TIM_GetTicketBoxRFIDInfo)(char cTicketBoxNo, tTIMRFIDInfo* pRFIDInfo, tTIMDevReturn *pDevStatus);
//设置票箱RFID信息
AFC_API_DLL_IMP int(TIM_SetTicketBoxRFIDInfo)(char cTicketBoxNo, tTIMRFIDInfo* pRFIDInfo, tTIMDevReturn *pDevStatus);
//读取票箱数量信息
AFC_API_DLL_IMP int(TIM_GetTicketBoxCountInfo)(char cTicketBoxNo,tTIMCardNum * pCardNum, tTIMDevReturn *pDevStatus);
//设置票箱数量信息
AFC_API_DLL_IMP int(TIM_SetTicketBoxCountInfo)(char cTicketBoxNo,tTIMCardNum* pCardNum,tTIMDevReturn *pDevStatus);
//更换票箱
AFC_API_DLL_IMP int(TIM_ReplaceTBox)(char cBoxNo, char cBoxMode, tTIMDevReturn* pDevStatus);
//读取序列号
AFC_API_DLL_IMP int(TIM_GetSN)(char* cModuleSN, tTIMDevReturn *pDevStatus);
//关闭通讯
AFC_API_DLL_IMP int(TIM_CommClose)();
//设置模块工作模式
AFC_API_DLL_IMP int(TIM_SetWorkMode)(char cMode, tTIMDevReturn* pDevStatus);
//模块测试设置
AFC_API_DLL_IMP int(TIM_TestSetting)(char cTestType, tTIMModuleStatus* pModuleStatus, tTIMDevReturn* pDevStatus);
/**
 ...
 * @return             [成功返回0 其余失败]
 */
AFC_API_DLL_IMP int TIM_TrayContral(int p_iTrayType,int p_iCommand,tTIMDevReturn* pDevStatus);
