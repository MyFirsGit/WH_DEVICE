...
// 	1	Int	0	成功
// 	2	Int	1	串口打开出错
AFC_API_DLL_IMP int BND_CommOpen(UINT uiCommPort, UINT uiBaudRate, tBNDDevReturn *pDevStatus);
...
// 	1	Int	0	成功
// 	2	Int	1	关闭错误
AFC_API_DLL_IMP int BND_CloseComm(tBNDDevReturn *pDevStatus);	
...
// 	1	Int	0	成功
// 	2	Int	1	传入参数为空
AFC_API_DLL_IMP int  BND_Init(tBndInitNumInfo *pNumInfo, tBNDDevReturn * pDevStatus); 
...
// 
// 	返回值说明见下表。
AFC_API_DLL_IMP int BND_Reset(tBndInitNumInfo *pNumInfo, tBNDDevReturn *pDevStatus);
...
// 	1	Int	0	成功
// 	2	Int	1	传入参数为空
AFC_API_DLL_IMP int BND_GetDevStatus(tBndBoxStatus *pBndBoxStatus, tBNDDevReturn * tBNDDevReturn);
...
// 	返回值说明见下表。
// 	1	int	0：执行成功；1，执行失败。
AFC_API_DLL_IMP int BND_GetAuditData(tBndAuditCashNum *pBndAuditCashNum, tBNDDevReturn *pDevStatus);
...
// 	返回值见表
// 	1	int	0：执行成功；1，执行失败。
AFC_API_DLL_IMP int BND_GetBillBoxInfo(tBNDBillBoxInfo *pBillBoxInfo, tBNDDevReturn *pDevStatus);
...
// 	返回值见下表。
// 	1	int	0：执行成功；1，执行失败。
AFC_API_DLL_IMP int BND_SetBillBoxInfo(UINT uiSlotID, tBNDBillBoxInfo *pBillBoxInfo, tBNDDevReturn *pDevStatus);
...
// 	返回值说明见下表。
// 	1	int	0：执行成功；1，执行失败。
AFC_API_DLL_IMP int BND_ChangeBill(tBNDChangeNum *pChangeNum, tBNDDevReturn *pDevStatus);
...
// 	序号	类型	说明
// 	1	int	0：执行成功；1：执行失败。
AFC_API_DLL_IMP int BND_StartReplaceBox(UINT uiBoxType, UINT uiBoxNo, tBNDDevReturn *pDevStatus);
...
// 	1	Int	0	成功
// 	2	Int	1	传入参数为空
AFC_API_DLL_IMP int BND_StopReplaceBox(UINT uiBoxType, UINT uiBoxNo, tBNDDevReturn *pDevStatus);
...
// 	返回值说明见下表。
// 	1	int	0：执行成功；1，执行失败。
AFC_API_DLL_IMP int BND_GetSerialNumber(unsigned char * pSn, tBNDDevReturn * pDevStatus);
...
// 	返回值说明见下表。
// 	1	int	0：执行成功；1，执行失败。
AFC_API_DLL_IMP int BND_GetFWVerdion (unsigned char * pVersion, tBNDDevReturn * pDevStatus);
...
// 	1	Int	0	成功
// 	2	Int	1	传入参数为空
AFC_API_DLL_IMP int  BND_Cancel (tBNDDevReturn *p_psStatus);
...
// 	返回值见下表。
// 	1	int	0：执行成功；1，执行失败。
AFC_API_DLL_IMP int BND_WriteRFIDInfo(UINT uiSLotID, tBndRfidInfo* pRFIDInfo, tBNDDevReturn * pDevStatus);
...
// 	返回值说明见下表。
// 	1	int	0：执行成功；1，执行失败。
AFC_API_DLL_IMP int BND_ReadRFIDInfo(UINT uiSLotID, tBndRfidInfo* pRFIDInfo, tBNDDevReturn * pDevStatus);
