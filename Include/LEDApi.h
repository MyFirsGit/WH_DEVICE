...
#pragma pack(pop)
	//11.4.1　通信建立
	 AFC_API_DLL_IMP int  LED_CommOpen(UINT uiCommPort, UINT uiBaudRate);
	//11.4.2　初始化
	 AFC_API_DLL_IMP int  LED_Init(tLEDDevReturn*  pDevStatus);
	//11.4.3　模块复位
	 AFC_API_DLL_IMP int  LED_Reset(tLEDDevReturn*  pDevStatus);
	//11.4.4　下载显示信息
	//此命令用于将需要显示的信息下载到LED控制板上或保存在内存变量中
	 AFC_API_DLL_IMP int  LED_Download(tLED_Download_Text_Para pLED_Download_Para,tLEDDevReturn *pDevStatus);
	//11.4.5　显示信息
	//此命令用于根据选择的序号将保存在LED控制板上或内存变量中的字符串内容进行显示。
	//要求显示内容切换时，需立即显示
	//UINT	nSerialNO	In	需显示的数据编号。与tLED_Download_Text_Para:: nSerialNO对应。
	 AFC_API_DLL_IMP int  LED_Display(UINT nSerialNO, tLEDDevReturn * pDevStatus);
	//11.4.6　获取模块状态
	 AFC_API_DLL_IMP int  LED_GetDevStatus(tLEDModuleStatus *pModuleStatus,tLEDDevReturn *pDevStatus);
	//11.4.7　获取模块驱动版本信息
	 AFC_API_DLL_IMP int LED_GetVersion(char* cFirmWareVersion, char* cDriverVersion, tLEDDevReturn *pDevStatus);
	//11.4.8　通信关闭
	 AFC_API_DLL_IMP int LED_CommClose();
